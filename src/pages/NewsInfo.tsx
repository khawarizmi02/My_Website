...
const NewsContent = ({ newsId }: { newsId: string }) => {
  ...
  return (
    <section className={`${styles.flexCenter} px-7 sm:px-7 flex-col relative`}>
      {filteredNews && (
        <>
          <img
            src={urlFor(filteredNews.image).url()}
            className="px-none sm:px-3 py-3 w-full max-h-[450px] object-cover rounded-[15px]"
          />
          <div className="grid grid-cols-1 sm:grid-cols-[80%,20%] w-full">
            <div className="flex flex-col">
              <div className="flex items-center space-x-4 pt-6">
                <RxCalendar className="w-[40px] h-[40px]" />
                <div className="font-poppins font-medium text-[20px] text-black ">
                  {formatDate(filteredNews._updatedAt)}
                </div>
              </div>
              ...
              <div>
                {"subtopics" in filteredNews &&
                filteredNews.subtopics.length > 0 ? (
                  filteredNews.subtopics.map((topic) => (
                    <div className="flex flex-col pr-6 pb-6">
                      <div
                        className={`${styles.point} border-b-2 border-blackBlur pb-2`}
                      >
                        {topic.subtopicName}
                      </div>
                      <div className="font-poppins text-black text-[20px] py-5">
                        <RenderBlockContent
                          blocks={topic.subtopicDescription}
                        />
                      </div>
                    </div>
                  ))
            ...
            <div className="flex flex-col p-3">
              <div className="font-poppins font-medium text-black text-[20px] text-center">
                {" "}
                Read more news and tips from us:
              </div>
              <div>
                {listedNews.map((item) => (
                  <div className="py-5 min-w-[120px]">
                    ...
                    <div className="pt-3 font-poppins font-medium capitalize text-[20px] text-black">
                      {" "}
                      {item.topic}{" "}
                    </div>
                    <div className="py-none font-poppins font-extralight text-[15px] text-black">
                      {" "}
                      {formatDate(item._updatedAt)}{" "}
                    </div>
                    <Link
                      to={`/news/${item.slug.current}`}
                      className="font-poppins font-bold text-[15px] text-black"
                    >
                      {" "}
                      Read More +
                    </Link>
